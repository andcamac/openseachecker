...
import { apiKey, osFetch, pick, toMs, collectionLinks } from "../lib/opensea.js";
import { signalFor } from "../lib/signal.js";
...
export default async function handler(req, res) {
  ...
  const future = all.filter((s) => s.startTime && s.startTime > now);
  const nextStage = drop.next_stage ? fmtStage(drop.next_stage) : future[0] || null;
  const common = {
    ...base,
  ...
  // Nothing open. Either it hasn't started yet, or the whole drop is over — OpenSea's
  // "upcoming" calendar keeps listing drops whose stages have all closed.
  if (open.length === 0) {
    return res.status(200).json({ ...common, status: future.length ? "not_open" : "ended", endedAt: all.length ? Math.max(...all.map((s) => s.endTime || 0)) || null : null });
  }
