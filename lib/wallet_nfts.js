...
async function evmChain(key, address, chain, deadline, collection) {
  ...
  for (; pages < MAX_PAGES; pages++) {
    ...
    if (!ok) {
      ...
      if (status === 404) break;
      return { chain, error: body.detail || body.error || body.raw || `OpenSea HTTP ${status}`, nfts: out, pages, truncated: true };
    }
...
// GET /api/nfts?route=list&address=<0x… or base58>[&collection=slug][&chains=ethereum,base][&debug=1]
export default async function handler(req, res) {
  ...
  try {
    ...
    return finish(await Promise.all(chains.map((c) => cached(`wnfts:${c}:${address}:${collection}`, 60_000, () => evmChain(key, address, c, deadline, collection)))));
  } catch (err) {
