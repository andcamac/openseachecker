...
// Chains OpenSea indexes. This list was missing robinhood, abstract, ape_chain and ink —
// which is how a wallet holding 20 of a Robinhood-chain drop came back empty even with
// chains=all. An identifier OpenSea doesn't know simply 404s and is skipped, so erring
// wide costs nothing but a request.
export const EVM_CHAINS = [
  "ethereum", "base", "matic", "arbitrum", "optimism", "zora", "blast",
  "robinhood", "abstract", "ape_chain", "ink", "soneium", "unichain", "b3",
  "shape", "sei", "avalanche", "berachain", "flow_evm", "ronin",
];
// The handful worth scanning when someone just wants "the usual suspects".
export const COMMON_CHAINS = ["ethereum", "base", "matic", "arbitrum", "optimism"];
...
// Ask OpenSea which chain a collection lives on. One request, an exact answer, and it
// works for chains that aren't in the list above at all — which is the whole point.
async function collectionChain(key, slug) {
  const { ok, body } = await osFetch(key, `/collections/${encodeURIComponent(slug)}`);
  if (!ok) return null;
  const c = (body.contracts || []).find((x) => x && x.chain);
  return c ? String(c.chain).toLowerCase() : null;
}
...
export default async function handler(req, res) {
  ...
  const finish = (results) => {
    ...
    return res.status(200).json({
      ...
      resolvedChain: resolved || null,     // set when a collection told us where to look
      nfts,
      count: nfts.length,
  ...
  try {
    ...
    // One chain by default. Scanning them all is a deliberate choice (chains=all), not the
    // fallback — it is many times the requests, and it is how we used to trip the rate limit.
    const want = String(req.query?.chains || "").trim().toLowerCase();
    let chains = !want ? [DEFAULT_CHAIN]
      : want === "all" ? EVM_CHAINS
      : want === "common" ? COMMON_CHAINS
      : want.split(",").map((c) => c.trim()).filter(Boolean);
    if (!chains.length) return res.status(400).json({ error: `chains must be "all", "common", or chain identifiers` });

    // If a collection was named, OpenSea knows its chain — so ask, rather than searching
    // every chain hoping to stumble on it. This is the only way to find a collection on a
    // chain we don't list, and it turns the whole scan into two requests.
    let resolvedChain = null;
    if (collection) {
      resolvedChain = await collectionChain(key, collection);
      if (resolvedChain) { chains = [resolvedChain]; resolved = resolvedChain; }
    }
